...
namespace GeometryClasses
{
    internal class Point3D : Point
    {
        ...
        public Point3D cross_prod(Point3D p1, Point3D p2) => new Point3D(new double[3]
            {p1.x[1] * p2.x[2] - p1.x[2] * p2.x[1],
            p1.x[2] * p2.x[0] - p1.x[0] * p2.x[2],
            p1.x[0] * p2.x[1] - p1.x[1] * p2.x[0]}
        );

        public Point3D cross_prod(Point3D p) => cross_prod(this, p);

        // (a*[b x c])
        public double mix_prod(Point3D p1, Point3D p2, Point3D p3) => mult(p1, cross_prod(p2, p3));
